  ...
  <div class="row">
    <div class="col-2">
      <div class="list-group">
         ...
         ?>
         <a href="allowuser.php" class="list-group-item list-group-item-action active">คำขอลงทะเบียนร้านค้า</a>
...
<?php 
    ...
    $sql = "select * from verify_store where status = 'Not verify' limit {$start} , {$perpage}";
    $query = mysqli_query($con, $sql);
    $query2 = mysqli_query($con,'SELECT * FROM verify_store');
...
?>
        <div class="col-md-3">
          <div class="card mb-4 shadow-sm">
            ...
            <div class="card-body">
              <p class="card-text">User : <?=$result['username']?></p>
              <div class="d-flex justify-content-between align-items-center" style="margin-top:20px">
                <div class="btn-group">
                  <a type="button" class="btn btn-success" href="system/addverify.php?id=<?=$result['user_id']?>&status=1&users=<?=$result['username']?>">Add</a>
                </div>
                <a type="button" class="btn btn-danger" href="system/addverify.php?id=<?=$result['user_id']?>&status=0&users=<?=$result['username']?>">Delete</a>
              </div>
              </div>
            </div>
          </div>  
